...
static void download_one_url(const char *url)
{
	...
	fprintf(stderr, "download_one_url start\n");
...
//			bb_error_msg_and_die("bad redirection (no Location: header from server)");
	...
	if (!(option_mask32 & WGET_OPT_SPIDER)) {
		...
		fprintf(stderr, "exiting from retrieve_file_data to download_one_url");
...
# define LONGOPTS
	...
	fprintf(stderr, "download_one_url done\n");
	if (G.output_fd >= 0)
		xclose(G.output_fd);
